<template>
      <div  class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
        <!-- {{item}} -->
            <div class="p-3">
              <a :href="item.url" class="hover:text-black font-bold text-l mb-1 text-gray-600 text-center">{{item.bookMark_name}}</a>
              <div class="flex items-center justify-center mt-2 gap-x-1">
                <button @click="liked" class="like-btn group " 
                :class="{
...
export default {
  ...
  computed: {
    ...
    alreadyLiked(){
      // return {
      //   "bookmark-item-active" : this._userLikes?.indexOf(this.item.id)>-1 // ! false it gives us array, but we need true or false
      // }
...
// console.log(likes);
      ...
      .then(()=>{  //! if ulanmasak without like_res = ()=>
        // this.$store.commit('setLikes', this.item.id)
